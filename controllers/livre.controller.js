...
export const getLivres = async (req, res) => {
 try { const cat = await Livre.find().populate('auteurs')
 .populate('specialite')
 .populate('maised');
;
...
export const getLivreByID = async (req, res) => {
 try {
 const liv = await Livre.findById(req.params.id).populate('auteurs').populate('specialite').populate('maised');
...
isbn:isbn,titre:titre,annedition:annedition,prix:prix,qtestock:qtestock,couverture:couverture,
specialite:specialite,maised:maised,auteurs:auteurs })
 try {
 await newLivre.save();
...
de livre avec un id: ${id}`);
 const liv1 = {
isbn:isbn,titre:titre,annedition:annedition,prix:prix,qtestock:qtestock,
couverture:couverture,specialite:specialite,maised:maised,auteurs:auteurs, _id: id
};
